...
func TestGetMaterializedBackups(t *testing.T) {
	...
	_, err0 := createMaterializedBackup(bid, "abc", time.Now(), time.Now(), "any")
	assert.Nil(t, err0, "err")
	bid = strconv.Itoa(rand.Int())
	_, err0 = createMaterializedBackup(bid, "def", time.Now(), time.Now(), "any")
	assert.Nil(t, err0, "err")
	bid = strconv.Itoa(rand.Int())
	_, err0 = createMaterializedBackup(bid, "ghi", time.Now(), time.Now(), "any")
	assert.Nil(t, err0, "err")
...
func TestGetFilteredMaterializedBackups(t *testing.T) {
	initDB()
	bid := strconv.Itoa(rand.Int())
	_, err0 := createMaterializedBackup(bid, "123", time.Now(), time.Now(), "any")
	assert.Nil(t, err0, "err")
	bid = strconv.Itoa(rand.Int())
	_, err0 = createMaterializedBackup(bid, "456", time.Now(), time.Now(), "any")
	assert.Nil(t, err0, "err")
	bid = strconv.Itoa(rand.Int())
	_, err0 = createMaterializedBackup(bid, "456", time.Now(), time.Now(), "any")
	assert.Nil(t, err0, "err")
